...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    result = V.view(bs,seq_len,self.all_head_size) 
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    #ßattention_output = self.attention_dropout(attention_output)
    ...
    #interm_output = self.out_dense(interm_output)
    #interm_output = self.out_layer_norm(interm_output)
    #interm_output = self.out_dropout(interm_output)
   
    
    # another add-norm layer
    #interm_output = self.add_norm(attention_output, interm_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    
    #return interm_output
    attention_output = self.self_attention(hidden_states, attention_mask)
    attention_out = self.add_norm(hidden_states, attention_output, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
